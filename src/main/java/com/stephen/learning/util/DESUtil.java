...
public class DESUtil {
     ...
     * @param password 加密密码，长度不能够小于8位
     * @param data 待加密字符串
     * @param ivParameter IV向量必须为8位
     * @return 加密后内容
     */
    public static String encryptCBC(String password, String data,String ivParameter) {
        if (password== null || password.length() < 8) {
            throw new RuntimeException("加密失败，key不能小于8位");
        }
        if (data == null)
     ...
     * @return 解密后内容
     */
    public static String decryptCBC(String password, String data,String ivParameter) {
        if (password== null || password.length() < 8) {
            throw new RuntimeException("加密失败，key不能小于8位");
        }
        if (data == null)
